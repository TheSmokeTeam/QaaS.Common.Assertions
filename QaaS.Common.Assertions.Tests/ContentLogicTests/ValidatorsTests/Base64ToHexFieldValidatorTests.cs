...
{
    private static readonly Base64ToHexFieldValidator FieldValidator = new(
        new Base64ToHexFieldValidationConfig()
    );
    ...
    public void TestValidateBase64ToHex_FieldValueIsNull_ThrowsArgumentNullException()
    {
        var exception = Assert.Throws<ArgumentNullException>(() =>
            FieldValidator.Validate(null, "A1")
        );
    ...
    {
        var exception = Assert.Throws<ArgumentNullException>(() =>
            FieldValidator.Validate("cGFzc3dvcmQ=", null)
        );
