...
public class ValidateHermeticMetricsByInputOutputPercentage
    : BaseAssertion<ValidateHermeticMetricsByInputOutputPercentageConfig>
{
    ...
    )
    {
        ...
        var collectorData = sessionDataList
            .AsSingle()
            .GetOutputByName(Configuration.MetricOutputSourceName!)
            .CastCommunicationData<JsonObject>()
            .Data;
        var inputValue = GetLatestMetricAndPrintAssertion(
            Configuration.InputMetricName!,
            collectorData,
            "Input"
        );
        var outputValue = GetLatestMetricAndPrintAssertion(
            Configuration.OutputMetricName!,
            collectorData,
            "Output"
        );
        var processingValue = GetLatestMetricAndPrintAssertion(
            Configuration.ProcessMetricName,
            collectorData,
            "Process"
        );
        var combineValue = GetLatestMetricAndPrintAssertion(
            Configuration.CombineMetricName,
            collectorData,
            "Combine"
        );
        var filteredValue = GetLatestMetricAndPrintAssertion(
            Configuration.FilteredMetricName,
            collectorData,
            "Filtered"
        );
        var splitValue = GetLatestMetricAndPrintAssertion(
            Configuration.SplitMetricName,
            collectorData,
            "Split"
        );

        // Based on hermetics formula
        var metricsHermeticsResultPercentage =
            inputValue + splitValue != 0
                ? (outputValue + processingValue + combineValue + filteredValue)
                    * 100
        ...
        if (
            double.IsPositiveInfinity(metricsHermeticsResultPercentage)
            && double.IsPositiveInfinity(outputInputResultPercentage)
        )
            return true;
        if (
            double.IsInfinity(metricsHermeticsResultPercentage)
            || double.IsInfinity(outputInputResultPercentage)
        )
            return false;
        return Math.Abs(metricsHermeticsResultPercentage - outputInputResultPercentage)
            < Configuration.Tolerance;
    }

    private long GetLatestMetricAndPrintAssertion(
        string? metricName,
    ...
    private long GetLatestMetricValueFromMetricsResults(
        string metricName,
        IList<DetailedData<JsonObject>> metricsResults
    )
    {
        var currentMetricResults = metricsResults
            .Where(metricResult =>
            {
                var metricResultName = JsonNode
                    .Parse(metricResult.Body?[Metric]?.ToString()!)
                    ?[NameProperty]?.ToString();
                return metricResultName != null && metricResultName == metricName;
            })
            .ToList();
        if (currentMetricResults.Count == 0)
            return 0;
        var latestCurrentMetricResult = currentMetricResults.MaxBy(detailedData =>
            detailedData.Timestamp
        );
        var metricValue = Convert.ToInt64(
            latestCurrentMetricResult?.Body?[ValueProperty]?.GetValue<string>()
