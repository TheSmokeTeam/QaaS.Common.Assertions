...
public class HermeticByExpectedOutputCountInRange
    : BaseAssertion<HermeticByExpectedOutputCountInRangeConfiguration>
{
    /// <inheritdoc />
    ...
    )
    {
        ...
        AssertionMessage =
            $"The sum of the count of the outputs {string.Join(", ", Configuration.OutputNames!)} is "
            + $"{outputCount}. The maximum limit is {Configuration.ExpectedMaximumCount}, "
            + $"The Minimum limit {Configuration.ExpectedMinimumCount}.";
        return Configuration.ExpectedMinimumCount <= outputCount
            && outputCount <= Configuration.ExpectedMaximumCount;
    }
}
