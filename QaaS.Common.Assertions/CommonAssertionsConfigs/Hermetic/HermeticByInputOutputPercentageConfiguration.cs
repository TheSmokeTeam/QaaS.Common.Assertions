...
[
    Description(
        "Checks for IO hermetics by comparing the count of multiple outputs in a given session with certain"
...
public record HermeticByInputOutputPercentageConfiguration
{
    ...
    [
        Description(
            $"Whether the given inputs in {nameof(InputNames)} are actually another outputs list"
        ),
        DefaultValue(false)
    ]
    ...
    [
        Description(
            "Specifies the strategy the mathematical rounding method used for calculating the expected number of"
