...
public class DelayByAverage : BaseAssertion<DelayByAverageConfiguration>
{
    ...
    )
    {
        ...
        {
            AssertionMessage =
                $"No outputs found in {output.Name}, meaning there was no delay to be measured";
            return true;
        }

        // Get average input injection time — sum raw ticks first, then divide once to avoid per-item truncation.
        BigInteger averageInputTime = 0;
        input.Data.ForEach(item =>
            averageInputTime += !item.Timestamp.HasValue
                ? throw new NotSupportedException(
                    "Encountered input with no timestamp, can't calculate delay. "
                        + $"\n Input with no timestamp: {item.Body}"
                )
                : item.Timestamp.Value.Ticks
        );
        // Divide total ticks by count, then convert to whole milliseconds once.
        averageInputTime = averageInputTime / inputCount / TimeSpan.TicksPerMillisecond;
        Context.Logger.LogDebug(
            "Average input time is {AverageInputTime}",
            new DateTime(TimeSpan.FromMilliseconds((double)averageInputTime).Ticks).ToString(
                DateTimeLogFormat
            )
        );
        ...
        output.Data.ForEach(item =>
            averageOutputTime += !item.Timestamp.HasValue
                ? throw new NotSupportedException(
                    "Encountered output with no timestamp, can't calculate delay. "
                        + $"\n Output with no timestamp: {item.Body}"
                )
                : item.Timestamp.Value.Ticks
        );
        // Divide total ticks by count, then convert to whole milliseconds once.
        averageOutputTime = averageOutputTime / outputCount / TimeSpan.TicksPerMillisecond;
        Context.Logger.LogDebug(
            "Average output time is {AverageOutputTime}",
            new DateTime(TimeSpan.FromMilliseconds((double)averageOutputTime).Ticks).ToString(
                DateTimeLogFormat
            )
        );
        ...
        if (averageDelayMilliseconds < negativeDelayBufferMilliSecondsAsNegativeNumber)
            throw new NegativeDelayException(
                $"Average delay is {averageDelayMilliseconds} milliseconds, delay cannot be a negative number"
        ...
        AssertionMessage =
            $"Average Delay between all inputs to all outputs is {averageDelayMilliseconds} milliseconds, "
