...
public class HttpStatus : BaseAssertion<HttpStatusConfiguration>
{
    ...
    )
    {
        ...
        var outputs = Configuration.OutputNames!.Select(output =>
            sessionData.GetOutputByName(output)
        );
        ...
        if (numberOfItemsWithNoStatus > 0)
            throw new ArgumentException(
                $"{numberOfItemsWithNoStatus} output items have no Http Status at all, check your"
                    ...
                    + string.Join(
                        ", ",
                        itemsWithNoStatusPerOutput
                            .ToList()
                            .Select(kv => $"output {kv.Key} - {kv.Value} items")
                    )
            );

        // Some output items had an incorrect status
        var numberOfItemsWithIncorrectStatus = itemsWithIncorrectStatusPerOutput.Sum(output =>
            output.Value
        );
        if (numberOfItemsWithIncorrectStatus > 0)
        {
            ...
            AssertionTrace =
                ...
                + string.Join(
                    ", ",
                    incorrectStatusesCount
                        .ToList()
                        .Select(kv => $"status code {kv.Key} - {kv.Value} times")
                )
                + ".\n Each output and the number of items that have an incorrect status in it are as follows: "
                + string.Join(
                    ", ",
                    itemsWithIncorrectStatusPerOutput
                        .ToList()
                        .Select(kv => $"output {kv.Key} - {kv.Value} items")
                );
            return false;
        ...
        AssertionMessage =
            $"All configured outputs arrived with status {Configuration.StatusCode!.Value}";
        return true;
    }
}
